...
int sort_first_q(struct seq_file *s, void *v)
{
	int i, j, k;
	char temp;
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		for(j = 0; j < 3999; j ++)
		{
			for( k = j+1; k < 4000; k++)
			{
				if( *((char *)(qs->data[0]+j)) < *((char *)(qs->data[0]+k))){
					temp = *((char *)(qs->data[0]+j));
					*((char *)(qs->data[0]+j)) = *((char *)(qs->data[0]+k));
					*((char *)(qs->data[0]+k)) = temp;
				}
			}
		}
...
{
	return single_open(filp, sort_first_q, NULL);
}
...
{
	proc_create_data("sort_first_quantum", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("sort_first_quantum", NULL /* parent dir */);
