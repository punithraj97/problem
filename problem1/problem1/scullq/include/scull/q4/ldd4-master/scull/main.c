...
int first_fifty_char(struct seq_file *s, void *v)
{
	int i, j;
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		if (mutex_lock_interruptible(&d->mutex))
			return -ERESTARTSYS;
		seq_printf(s, "first 50 characters of device %d",i);
		for(j = 0; j < 50; j++){
	       		seq_printf(s, "%c",*((char * )qs->data[0]+j));
		}
	       	seq_printf(s, "\n");
...
{
	return single_open(filp, first_fifty_char, NULL);
}
...
{
	proc_create_data("first_50_char", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("first_50_char", NULL /* parent dir */);
