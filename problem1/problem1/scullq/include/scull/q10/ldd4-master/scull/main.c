...
int blank_1_quantum(struct seq_file *s, void *v)
{
	int i, j;
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		if(!d) goto out;
		if(!d->data) goto out;
		if(!d->data->data) goto out;
		if(!d->data->data[0]) goto out;
		for(j=0;j<scull_quantum;j++)		
...
{
	return single_open(filp, blank_1_quantum, NULL);
}
...
{
	proc_create_data("blank_first_quantum", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("blank_first_quantum", NULL /* parent dir */);
