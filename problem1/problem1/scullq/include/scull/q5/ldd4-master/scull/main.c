...
int last_fifty_char(struct seq_file *s, void *v)
{	
	int i, j,qset,k,size,item,spos,qpos,quantu,remainder,rest;
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		if (mutex_lock_interruptible(&d->mutex))
			return -ERESTARTSYS;
		seq_printf(s, "last fifty characters of device %i\n",i);
		size=d->size;
		quantu=d->quantum;
		qset=d->qset;

		item=size/(quantu*qset);
		remainder=size%(quantu*qset);
		qpos=remainder/quantu;
		rest=remainder%quantu;
		if(!d) goto out;
		dptr=scull_follow(d,item);
		if(!dptr)  goto out;
		if(!dptr->data) goto out;
		if(!dptr->data[qpos]) goto out;
		if(rest<50) { 
		for(k=0;k<rest;k++)
		{	
		seq_printf(s, "%c ",*((char*)(dptr->data[qpos]+k)));
		}
		}
		else
		{ for(k=rest-50;k<rest;k++)
			seq_printf(s, "%c ",*((char*)(dptr->data[qpos]+k)));
		}
		seq_printf(s, "\n ");
		out:
...
{
	return single_open(filp,last_fifty_char, NULL);
}
...
{
	proc_create_data("last_50_char", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("last_50_char", NULL /* parent dir */);
