...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	int retval = 0;
         ...
         struct scull_dev* q=filp->private_data;
	/*
	...
	switch(cmd) {
	  ...
	  case SCULL_IOCSORTFIRSTQUANTUM:
	                                  if(!q) goto out;
	                                  if(!q->data) goto out;
	                                  if(!q->data->data) goto out;
	                                  if(!q->data->data[0]) goto out;
	   			  for(int i=0;i<scull_quantum-1;i++)
	   			  {
	   			  for(int j=i+1;j<scull_quantum;j++)
	   	              {  
	   	     if(*((char *)(q->data->data[0]+i))>*((char *)(q->data->data[0]+j)))
	   	    {
	   	    temp=*((char *)(q->data->data[0]+j));
	   		*((char *)(q->data->data[0]+j))=*((char *)(q->data->data[0]+i));   
	   		*((char *)(q->data->data[0]+i))=temp;
	   		}}}    
