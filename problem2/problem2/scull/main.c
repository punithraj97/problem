...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
	  ...
	  case SCULL_IOCSORTNTHQUANTUM:  
	  				  int n;
	  				  __get_user(n, (int __user *)arg);
	                                  if(!q) goto out;
	                                  if(!q->data) goto out;
	                                  if(!q->data->data) goto out;
	                                  if(!q->data->data[n]) goto out;
	   			  for(int i=0;i<scull_quantum-1;i++)
	   			  {
	   			  for(int j=i+1;j<scull_quantum;j++)
	   	              {  
	   	     if(*((char *)(q->data->data[n]+i))>*((char *)(q->data->data[n]+j)))
	   	    {
	   	    temp=*((char *)(q->data->data[n]+j));
	   		*((char *)(q->data->data[n]+j))=*((char *)(q->data->data[n]+i));   
	   		*((char *)(q->data->data[n]+i))=temp;
	   		}}}    
