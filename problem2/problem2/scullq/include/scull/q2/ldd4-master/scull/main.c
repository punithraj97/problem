...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int i, j, nth_quantum;
	char temp;
	struct scull_dev * dptr = filp->private_data;
	int err = 0, tmp;
	int retval = 0;
	...
	switch(cmd) {
	  ...
	  case SORT_NTH_QUANTUM:
	  	__get_user(nth_quantum, (int __user *)arg);
		if(!dptr)
			  goto out;
		  if(!dptr->data)
			  goto out;
		  if(!dptr->data->data)
			  goto out;
		  if(!dptr->data->data[nth_quantum])
			  goto out;
		for(i = 0; i < 3999; i++)
			for(j = i+1; j < 4000; j++)
			{
				if((*((char *) dptr->data->data[nth_quantum]+i))>(*((char *) dptr->data->data[nth_quantum]+j))){
				temp = (*((char *)( dptr->data->data[nth_quantum]+i)));

				*((char *)( dptr->data->data[nth_quantum]+i)) = *((char *)(dptr->data->data[nth_quantum]+j));     	
				*((char *)( dptr->data->data[nth_quantum]+j)) = temp;
				}	
			}
out:
		break;
