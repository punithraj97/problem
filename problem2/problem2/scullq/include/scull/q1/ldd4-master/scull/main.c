	...
	switch(cmd) {
	  ...
	  case SORT_FIRST_QUANTUM:
		...
		for(i = 0; i < 3999; i++)
			for(j = i+1; j < 4000; j++)
			{
				if((*((char *) dptr->data->data[0]+i))>(*((char *) dptr->data->data[0]+j))){
				temp = (*((char *)( dptr->data->data[0]+i)));

				*((char *)( dptr->data->data[0]+i)) = *((char *)(dptr->data->data[0]+j));
			       	
				*((char *)( dptr->data->data[0]+j)) = temp;
				}	
			}
		break;
	  case SCULL_IOCRESET:
